...
<Popover.Root>
  ...
  <Popover.Content
    align="start"
    class="flex flex-col overflow-hidden rounded-lg border border-border bg-popover p-0 text-popover-foreground"
    sideOffset={16}>
    {#each menuItems as item (item.label)}
      {@render item.renderer(item as Omit<MenuItem, 'renderer'>)}
    {/each}
  </Popover.Content>
</Popover.Root>
