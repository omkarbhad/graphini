...
export class AttachmentsContext {
  ...
  matchesAccept = (file: File): boolean => {
    ...
    for (const token of tokens) {
      ...
      if (token === 'video/*' && file.type.startsWith('video/')) return true;
      if (token === 'audio/*' && file.type.startsWith('audio/')) return true;
  ...
  add = (files: File[] | FileList) => {
    ...
    if (sized.length === 0 && accepted.length > 0) {
      const limitMB = this.maxFileSize ? (this.maxFileSize / (1024 * 1024)).toFixed(0) : '?';
      const rejectedNames = accepted
        .map((f) => `${f.name} (${(f.size / (1024 * 1024)).toFixed(1)}MB)`)
        .join(', ');
      this.onError?.({
        code: 'max_file_size',
        message: `File${accepted.length > 1 ? 's' : ''} too large (max ${limitMB}MB): ${rejectedNames}`
      });
      return;
    }
