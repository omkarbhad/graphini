<script lang="ts">
  ...
  import { kvSet } from '$lib/stores/kvStore';
  ...
  async function performValidation(code: string) {
    if (!code || code.trim().length < 5) {
      ...
      kvSet('editor', 'editorValidationError', null);
      return;
    }
    ...
    if (lastValidatedCode === code && lastValidationResult) {
      if (!lastValidationResult.valid && lastValidationResult.error) {
        kvSet('editor', 'editorValidationError', lastValidationResult.error);
      } else {
        kvSet('editor', 'editorValidationError', null);
      }
      return;
    ...
    try {
      ...
      if (!hasError) {
        ...
        kvSet('editor', 'editorValidationError', errorMessage);
      }
    } catch (e: any) {
      lastValidatedCode = code;
      lastValidationResult = { valid: false, error: e?.message || 'Invalid diagram syntax' };
      kvSet('editor', 'editorValidationError', e?.message || 'Invalid diagram syntax');
    }
  }
  let editorOptions = {
    minimap: {
